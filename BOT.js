...
fs.readdir('./Eventos/', (err, files) => {
        ...
        files.forEach(file => {
          ...
          client.on(nome, (...args) => evento.run(...args, client, database))
        });
});
client.on("message", async message => {
 ...
 const command = message.content.split(" ")[0].slice("/".length);
  try {
    let commandFile = require(`./Comandos/${command}.js`);
    commandFile.run(client, message, args, database, Discord)
  }catch(err) {
    console.log("Erro: " + err)
  }
})
client.login("Njg4NTUxNzUyMjE5MzYxMzAx.Xm19zg.L-m5h8WVa3YgqrK5rXDRh8GOQNU")
