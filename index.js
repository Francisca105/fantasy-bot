...
client.on("raw", async dados => {    
  ...
  if(dados.d.message_id != "689896625597972498") return 

  let discord = require('discord.js')
  const servidor = client.guilds.get("494909323550654465")
  ...
  if(cooldown.is(membro.id)) return client.channels.get('688051751265632320').send(member + ' aguarde para poder fazer essa ação novamente!').then(msg =>{msg.delete(2000)})
  ...
  //if(servidor.channels.has(c => c.name(name.toLowerCase() + "-" + discriminator))) return client.channels.get('688051751265632320').send(member + ' Já tem um ticket aberto!').then(msg =>{msg.delete(2000)})
  //if(servidor.channels.forEach((channel) => channel.name == `${name.toLowerCase()}-${discriminator}`)) return console.log('gg')
...
//});
  //var bool = false
  //servidor.forEach(function (channel){channel.name(userName.toLowerCase() + "-" + userDiscriminator).then(bool = true)});
  //let channels = servidor.channels
  ////if(servidor.forEach((channel).name === name.toLowerCase() + "-" + discriminator)) return console.log('Sla')
  //if(channels.name = name.toLowerCase() + "-" + discriminator) return client.channels.get('688051751265632320').send(member + ' Já tem um ticket aberto!').then(msg =>{msg.delete(2000)})
  //if(servidor.channels.find('name', name.toLowerCase() + "-" + discriminator)) return console.log('Log')
  if(dados.t === "MESSAGE_REACTION_ADD"){
      if(dados.d.emoji.id === "688051406388592751") {
        cooldown.add(membro.id)
